    ...
    try {
        ...
        if (res.ok && data.token) {
            ...
            if (res.status === 403 && data.emailVerified === false) {
                ...
                const errorMsg = data.error || "Login failed. Please check your email and password.";
                console.error("Login failed:", { email, status: res.status, error: data.error });
                alert(errorMsg);
            }
        }
        ...
        alert("Server error. Backend not reachable. Please make sure the backend server is running.");
    }
}
...
async function signup() {
    ...
    try {
        ...
        if (res.ok && data.token) {
            ...
            alert(data.message || "Signup successful! You have been automatically logged in.");
            closeAuthModal();
            updateNavForAuth();
        } else {
